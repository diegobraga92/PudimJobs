...
- **Jobs missed:** ~40 job postings published during the 2h window were not
  ingested until the post-recovery reparse.
...
- **Data loss:** none. Every fetch's `raw_html` was stored, enabling full
...
| 16:00 | `POST /api/admin/sources/{id}/reparse` replays stored `raw_html` for the window. |
...
- **`raw_html` retention** made the outage recoverable without re-fetching.
- **Reprocessing API** (`/sources/{id}/reparse`) recovered all missed jobs.
...
| # | Action | Owner | Status |
|---|--------|-------|--------|
| 1 | Add pre-normalization validation: reject a scrape when `title`/`company` missing from **every** parsed item (fail fast, record error) | eng | Assigned |
...
- Retaining raw input (`raw_html`) is the single most valuable resilience
  investment — it converts an outage into a replay.
